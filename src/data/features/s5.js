const s5 = [
  {
    id: 1,
    icon: '/assets/feature_icons/graphic_programming.png',
    i18nKey: 's_detail_pages.s5_features.id1',
  },
  {
    id: 2,
    icon: '/assets/feature_icons/drag_teaching.png',
    i18nKey: 's_detail_pages.s5_features.id2',
  },
  {
    id: 3,
    icon: '/assets/feature_icons/plug_and_play.png',
    i18nKey: 's_detail_pages.s5_features.id3',
  },
  {
    id: 4,
    icon: '/assets/feature_icons/wireless_connection.png',
    i18nKey: 's_detail_pages.s5_features.id4',
  },
  {
    id: 5,
    icon: '/assets/feature_icons/remote_monitoring.png',
    i18nKey: 's_detail_pages.s5_features.id5',
  },
];

export default s5;
