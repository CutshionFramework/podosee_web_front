...
export default function ZuSeriesPage() {
  ...
  // 🌟 번역 데이터 가져오기
  const pageTitle = t(
    `zu_detail_pages.products.${idNumber}.pageTitle`
  );
  ...
  const commonImg = t(
    `zu_detail_pages.products.${idNumber}.commonImg`
  );
  const featureTitle = t(
    `zu_detail_pages.products.${idNumber}.feature_title`
  );
  const fieldTitle = t(
    `zu_detail_pages.products.${idNumber}.field_title`
  );

  const features = id === "5" ? zu5FeaturesData : zu3FeaturesData; // id가 '5'이면 zu5Data, 아니면 zu3Data

  const fieldListItem =
    id === "3" ? fieldListItemZu3 : fieldListItemZu; // id가 '3'이면 fieldListItemZu3, fieldListItemZu
